// Amazon RDS ap-northeast-1 のルート CA バンドル。
// https://truststore.pki.rds.amazonaws.com/ap-northeast-1/ap-northeast-1-bundle.pem
//
// node-postgres は既定のトラストストアで RDS の証明書を検証できないため、ここで CA を与える。
// ファイルとして置かずに文字列で持つのは、Amplify の SSR バンドルに確実に載せるため
// （outputFileTracing や実行時の cwd に依存させない）。公開情報なので秘匿の必要はない。
export const RDS_CA = `
-----BEGIN CERTIFICATE-----
